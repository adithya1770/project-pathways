...
const Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (data) {
        const jsRepo = data.filter(repo => 
          (repo.language === "JavaScript" || repo.language === "HTML" || repo.language === "CSS") 
          && repo.name !== "learn-native"
        );
        setRepos(jsRepo);
      }
    };
    ...
    <div className="flex flex-wrap justify-center">
      ...
      {repos.map((repo) => (
        <div key={repo.id} className='ibm-plex-mono-medium lg:h-72 lg:w-110 grad lg:ml-10 bg-white m-4 rounded-xl shadow-md hover:border-cyan-400'>
          ...
          <img src={repo.owner.avatar_url} className='lg:h-20 lg:w-20 lg:absolute absolute lg:ml-80 rounded-3xl lg:mt-24 h-12 w-12 ml-64 mt-20 border-4 border-black' alt="Owner avatar"/>
